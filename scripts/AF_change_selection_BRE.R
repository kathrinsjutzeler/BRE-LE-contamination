...
df <- gather(RefFre.AD, 'sample', 'AF', 5:23)
...
#Rename chroms
chrom.labs <- c('Chr. 1', 'Chr. 2', 'Chr. 3', 'Chr. 4', 'Chr. 5', 'Chr. 6', 'Chr. 7', 'Chr. WSR',
                'Chr. Z')
...
ggsave('Figure3a.jpg', width = 8, height =6, units = 'in', dpi =600, bg="white")
...
# Plot ln ratio ####
r_mean <- r2 %>%
  filter(!sampleID %in% c('BRE092921_f', 'BRE092921_m'), CHROM != 'SM_V10_MITO') %>%
  group_by(LCS) %>%
...
p_ratio <- ggplot(r_mean, aes(LCS, -mean_s)) +
  ...
  stat_regline_equation(label.y = 1.8, label.x = 1) +
...
p_selection <- ggplot(subset(sc3, CHROM != 'SM_V10_MITO'), aes(POS, smooth_BRE)) +
  #geom_line() + 
...
t <- sc3 %>%
  filter(CHROM == 'SM_V10_4', smooth_BRE < 0.06) #%>%
